...
public class OWLAPI3OntologyFactory extends OntologyFactory {
	...
	if ( ontology instanceof OWLOntology ) {
		...
		OWLOntology ontol= (OWLOntology) ontology;
	    onto.setFormalism( formalismId );
	    onto.setFormURI( formalismUri );
	    onto.setOntology( ontol );
	    onto.setURI( ontol.getOntologyID().getOntologyIRI().toURI() );
	    cache.recordOntology( onto.getURI(), onto );
	    cache.recordOntology( ontol.getOntologyID().getOntologyIRI().toURI(), onto );
	    return onto;
	} else {
	...
	OWLAPI3Ontology onto = getOntology(uri);
    ...
    private OWLAPI3Ontology getOntology(URI uri){
		OWLAPI3Ontology onto = cache.getOntologyFromURI( uri );
